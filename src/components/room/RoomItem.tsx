...
export default function RoomItem({ room }: { room: RoomListItem }) {
  return (
    <Link
      to={`/${room.roomCode}`}
      className="flex w-full cursor-pointer items-center justify-between gap-6 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:border-gray-200 hover:shadow-md"
    >
