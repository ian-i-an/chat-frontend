  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-7">
      <section className="border-y border-border bg-surface px-5 py-5">
        <h2 className="text-base font-black text-foreground">기본 정보</h2>
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                title="질문 삭제"
                className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full text-subtle-foreground transition-colors hover:bg-danger-soft hover:text-danger disabled:cursor-not-allowed disabled:text-gray-200"
              >
            ...
            <input
              ...
              disabled={isPending}
              className="mt-4 w-full rounded-lg border border-border bg-background px-4 py-3 text-sm font-semibold text-foreground transition-colors outline-none focus:border-primary-hover focus:bg-surface"
            />
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5">
              {question.options.map((option, optionIndex) => (
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-surface text-xs font-black text-muted-foreground ring-1 ring-gray-200">
                    {optionIndex + 1}
                  </span>
                  <input
              ...
              {question.options.length < 4 && (
                <button
                  ...
                  disabled={isPending}
                  className="flex cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-dashed border-gray-300 py-3 text-sm font-bold text-muted-foreground transition-colors hover:border-blue-300 hover:text-primary disabled:cursor-not-allowed"
                >
      ...
      <div className="flex items-center justify-end gap-3 border-t border-border pt-5">
        <Link
          to="/admin"
          className="rounded-lg border border-border bg-surface px-5 py-3 text-sm font-bold text-gray-600 transition-colors hover:bg-background"
        >
          취소
        </Link>
        <Button
